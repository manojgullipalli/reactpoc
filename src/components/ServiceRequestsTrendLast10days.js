...
const ServiceRequestsTrendLast10days = () => {
    const [chartData, setChartData] = useState({ days: [], openCounts: [], closedCounts: [] });
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.post('http://localhost/myproject2/backend/getServiceRequestsTrendLast10days.php', { cust_id: [] });
    ...
    const option = {
        title: {
            text: 'Service Requests Trend Last 10 Days',
            left: 'left',
            padding: 15,
            textStyle: {
                fontSize: 14
        ...
        legend: {
            data: ['Open Service Requests', 'Closed Service Requests'],
            left: 'right',
            padding: 30,
            textStyle: {
                fontSize: 10,
...
export default ServiceRequestsTrendLast10days;
